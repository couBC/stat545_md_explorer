...
a string is a list of character vectors
...
This separates the sentence by space into different characters?
...
Output of a string split is always a list. [[1]] is vector length one - by word
...
double brackets [[]] give me the contents of each car
single bracket [] gives the content of the first car
```{r}
fox %>% 
  str_split("") %>% 
  `[[`(1) %>% 
...
str_length tells how many characters in each entry e.g., apple = 5 characters
length() looks at how many fruits in the vector e.g., = 80 fruits
```{r}
str_length(fruit)
length(fruit)
```
...
#no news is good news - no output, so it passes the test.
```
...
#Italy is not here - regular expressions is case sensitive
```
...
#match=TRUE only returns those that match. rather than first, it gives you all match and non-match
```
...
#not so helpful because it gives us bar, try below with the brackets
```
...
#when something is in square brackets([]) it means NOT, so in this case, no vowels
#*means repeat til end of the word $ so this is asking for a country with no vowels which doesn't exist
```
