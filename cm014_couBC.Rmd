...
#this is an example of dependence - x and y are related
```
...
#this is an example of independence of variables - axel ratio does not predict quarter mile time - scatterplot doesn't tell you much. If you fit a line - it'll be mostly flat.
...
*the method below requires tidydata*
...
#the y value here being predicted is lifeExp based on log(gdpPercap)
...
```{r}
n <- c(500, 600, 700)
t <- tibble(gdpPercap = n)
predict(my_lm, newdata = t)
gapminder %>%  head()
```
...
#however can't extract info from this ggplot, it just does it but doesn't give values
```


## Broom

*prediction, interpretation, goodness of fit*
...
```{r}
augment(my_lm, newdata = tibble(gdpPercap=n))

#newdata - what you want predictions of y for when you have the above x (e.g., 400, 500...)
```
...
#get all the data you need to report in one place
