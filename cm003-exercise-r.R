...
preamble <- "I live in Arbutus"
## What data types do `my_commute` and `preamble` contain? Use `typeof`.
typeof(my_commute)
typeof(preamble)
#double means integer in R

## Let's use the `paste` function to paste out the preamble followed by
##  the commute time:
paste(preamble, my_commute)## Bring up the documentation for the `print` function.
paste(my_commute)
?paste
#big source of error is not executing/running an update, eg. updating a number
...
commute <- c(my_commute, 40, 15, 20, 25, 35)
commute
class(commute)
class(commute)
#vector is a list of numbers of the same type, can only hold one type of object e.g., numbers or characters
#string is in quotations
...
#double == sign compares one number to another
"hi"=="hi"
typeof(TRUE)
