...
```{r}
#still runs it and dplyer recognizes the common column 'x1'
...
#intersect(y, x) wont work (error) because in set functions the datasets have to have identical columns
...
bind_cols(y, z)
z
#in column binding it doesn't have to have any matching columns, it just appends its all together by columns, but they can't be same names as that column will be mushed together
```
...
```{r}
# union(flights2, weather) doesn't work
bind_rows(flights2, weather)
```

```{r}
#bind_cols(flights2, weather) doesn't work because different number of rows
```
